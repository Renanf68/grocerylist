...
const Lists = (props) => {
  ...
  function listRemove(listId) {
    databaseRef
      .child(`${listId}`)
      .remove()
      .then(() => setRemove({ status: false, list: {} }))
  ...
  return (
    <Fragment>
      <div className="tip">
        {cwidth < 500 && (
          <h6>
            Melhor visualização na horizontal <MdScreenRotation />
          </h6>
        )}
      </div>
      <div className="component-wraped">
        ...
        <div className="lists-legend-container">
          <p className="lists-legend">
            <FaEye /> Visualizar lista.
          </p>
          <p className="lists-legend">
            <FaRegCopy /> Copiar itens para uma nova lista.
          </p>
          <p className="lists-legend">
            * Será possível visualizar suas 12 últimas listas.
          </p>
        </div>
        {isLoading ? (
          <p>Carregando...</p>
